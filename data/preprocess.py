...
串成一条命令，可复现。同时做基本的数据清洗：过滤损坏图片（PIL verify）、
校验每张图的类别标签与目录名一致

运行：python data/preprocess.py
依次调用 build_dataset_50 / build_nutrition_db / build_labels / build_area_stats /
build_scene_split / build_mixed_plates，并做完整性校验
注意：第一步会清空重建 dataset_50cls/，已有的场景标注进度会一并丢失，重跑前先备份
'''
...
def main():
    ...
    # area_stats 要读 train 图算面积比，mixed 要读 test 真值，依赖前面几步
