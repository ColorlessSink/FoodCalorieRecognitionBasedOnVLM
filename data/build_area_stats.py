'''
预计算每类桶的面积比统计脚本（给分量估计做标定）
---
旧几何法用固定 plate_ref_px=900 做绝对标定，cm_per_px 与图像分辨率耦合（大图被当小图
处理 → 食物面积爆高）。诊断证实 area_ratio 与短边几乎不相关(corr=0.007)，本身已是
尺寸归一化的好信号，故改用 area_ratio 作"相对调制器"，以类先验为锚

标定方式：对训练集每类抽 5 张算 area_ratio，按"先验桶"(staple/dish/snack/light)统计中位数
...
# `python data/build_area_stats.py` 启动时 sys.path 只含 data/，补上项目根目录
...
def main():
    ...
    # 训练集 1500 张太大，每类抽 5 张算面积比（中位数对桶级标定足够稳）
