...
# 与 cli_demo.py 相同的坑：`python demo/web_demo.py` 启动时 sys.path 只有 demo/，补上项目根
...
def reset_meal(agent, chat_log):
    ...
    welcome = [{"role": "assistant", "text": agent._greeting(), "image": None}]
    return welcome, state_html(agent)


def build_app(use_llm=True):
    agent = new_agent(use_llm=use_llm)
    # 首屏直接展示欢迎语；它只属于 UI 初始消息，不写入 agent.history，
    # 避免影响后续意图识别和 LLM 上下文。
    welcome_log = [{"role": "assistant", "text": agent._greeting(), "image": None}]
    ...
    with gr.Blocks(title="卡路里识别智能体") as app:
        ...
        with gr.Row():
            with gr.Column(scale=5, elem_id="chat-col"):
                ...
                chat_state = gr.State(welcome_log)
                chat = gr.Chatbot(
                    value=to_display(welcome_log),
                    height=520,
                    label="对话",
                    buttons=["copy"],
                )
