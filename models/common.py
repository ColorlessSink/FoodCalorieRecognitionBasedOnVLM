...
通用配置与模型加载工具模块
---
把"读 config.yaml + 加载 Chinese-CLIP + 取类别表"收敛到一个公共模块，
所有 models/ 文件 import 它，保证口径一致

流程：
  1. load_config() 用绝对路径定位 config/config.yaml，不管从哪个目录运行都找得到
  2. load_clip() 返回 (model, processor)，复用小作业验证过的"本地快照优先"策略
     （避免联网校验超时）；返回前 .eval() 且不 .to(device)——device 由调用方决定，
     同一份加载结果既能用于推理也能用于 LoRA 训练
  3. load_classes() 读 classes_50.csv（或 20cls 的 classes.csv），返回 (names_zh, idxs)
'''
import os
import yaml
import torch
...
def load_clip(cfg=None, device=None):
    # 加载 Chinese-CLIP base 模型与 processor，返回 (model, processor)
    # model 不 .to(device)，由调用方决定（便于训练/推理复用）
    if cfg is None:
        cfg = load_config()
...
def load_classes(data_dir):
    ...
    for fn in ("classes_50.csv", "classes.csv"):     # 50 类叫 classes_50.csv，20 类叫 classes.csv
