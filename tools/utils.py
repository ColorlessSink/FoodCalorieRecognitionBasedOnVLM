'''
通用工具模块（区域分割、面积比、深度代理、密度先验）
---
这些是"无状态的视觉/数值工具"，被 portion_estimator 调用，本身不持有模型。
按大作业规定的目录结构（tools/reference_detector.py, depth_estimator.py, utils.py）

关键能力：
  - food_mask_by_saliency(): 用 OpenCV GrabCut/显著性 + 餐盘先验，把"食物前景"
    从盘/桌背景里抠出来。不用 OWL-ViT/Grounding DINO 的原因：题面允许二选一/
    三选一，且本机无这些大检测器的权重，OpenCV 前景分割本地可跑、零下载，
    作为"多食物解耦与定位"创新点的轻量实现
...
def depth_proxy(img_bgr, mask):
    # 轻量单目深度代理：真实深度估计(MiDaS/Depth Anything)权重几十 MB，
    # 本机下载不可控，且分量估计主要靠"面积×厚度×密度"，深度只做厚度修正。
