...
### 案例 10：糖醋排骨误判为爆炒腰花/可乐鸡翅/梅菜扣肉（混淆 2+2+2=6 次，最易错类）
...
### 案例 16：红烧肉误判为爆炒腰花（conf=0.263，触发门控）
- **样本**：`dataset_50cls/test/10_红烧肉/10_000697.jpg`
- **模型输出**：pred=爆炒腰花 conf=0.263；top3=[爆炒腰花 0.263, 宫保鸡丁 0.234, 京酱肉丝 0.168]
- **归因**：conf=0.263 低于门控阈值 max(0.3, 81.5%×0.5)=0.407，触发反问。top3 概率分散（0.263/0.234/0.168），说明模型完全不确定。这张红烧肉图可能非典型（如酱色浅、肉块小），与多个酱色肉类都部分相似。
